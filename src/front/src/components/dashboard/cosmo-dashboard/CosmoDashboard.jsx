...
function CosmoDashboard({ prompt }) {
  ...
  useEffect(() => {
    ...
    const getTip = async () => {
      setCosmoTip(null);
      const tip = await cosmoTipCall(prompt);
      setCosmoTip(tip);
    };
  ...
  return (
    <div className="flex flex-col justify-center items-start h-70 w-70 overflow-hidden absolute cosmo-component">
      <div className="flex items-center justify-center rounded-t-2xl rounded-s-2xl w-8/9 p-3 h-20 max-h-20 ms-1 relative cosmoMessage">
        {cosmoTip ? (
          <p className="text-xs text-center">{cosmoTip.output}</p>
        ) : (
          <LoaderMini />
        )}
