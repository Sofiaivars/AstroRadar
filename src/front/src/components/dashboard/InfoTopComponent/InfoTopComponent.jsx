  ...
  useEffect(() => {
    if (userLocation) {
      const getLocateInfo = async () => {
        ...
        setLocateString(
          `${reverseGeocodingData.address.quarter}, ${reverseGeocodingData.address.city}, ${reverseGeocodingData.address.country}`
        );
      };
      getLocateInfo();
  ...
  return (
    <div className="flex items-center justify-between rounded-xl p-2 mb-1 w-full self-start borde-con-degradado">
      ...
      <div className="flex gap-1 items-center">
        ...
        {satCounter ? (
          <NumberFlow value={satCounter} format={{ minimumIntegerDigits: 4 }} />
        ) : (
          <LoaderMini />
        )}
      </div>
